#!/usr/bin/env python3
"""
FINAL Professional Dashboard Generator - Fixed Layout
====================================================

Creates the definitive professional dashboard with:
- Proper pie chart spacing (explode and shadow)
- Smaller, professional header text
- Fixed bottom bar chart label visibility
- Significant spacing between all charts
- Clean, interview-ready presentation

Created by: Salomón Santiago Esquivel
Usage: python create_final_professional_dashboard.py
"""
...
class FinalProfessionalDashboard:
    """Create the final, perfectly spaced professional dashboard"""
    
    def __init__(self):
        self.db_path = "../data/sales_data.db"
        self.output_dir = "../visualizations/final_professional"
    ...
    def load_data(self):
        ...
        try:
            ...
            print("Data loaded successfully for final professional dashboard")
        except Exception as e:
            print(f"Error loading data: {e}")
            
    def create_perfect_professional_dashboard(self):
        """Create the final professional dashboard with perfect spacing"""
        print("Creating final professional dashboard with perfect spacing...")
        
        # LARGER figure size for much better spacing
        fig = plt.figure(figsize=(20, 14))  # Increased from 16x10
        fig.patch.set_facecolor('white')
        
        # Enhanced grid layout with MORE spacing
        gs = fig.add_gridspec(3, 4, 
                            height_ratios=[0.25, 1.2, 1.2],  # Smaller header ratio
                            width_ratios=[1, 1, 1, 1],
                            hspace=0.45,  # MUCH more vertical space
                            wspace=0.3,   # More horizontal space
                            top=0.90,     # More space from top
                            bottom=0.12,  # More space from bottom
                            left=0.08,    # More left margin
                            right=0.92)   # More right margin
        
        # SMALLER, more professional title
        ...
        for i, (title, value, color) in enumerate(kpis):
            ...
            ax.text(0.5, 0.65, value, ha='center', va='center', 
                   fontsize=16, fontweight='bold', color=color)  # Smaller font
        ...
        # Chart 1: Revenue by Category (Middle Left) - Better spacing
        ...
        ax1.set_title('Revenue Potential by Category', fontsize=16, fontweight='bold', 
                     pad=25, color=self.colors['dark'])  # More padding
        ...
        for bar, value in zip(bars1, revenue_data.values):
            ax1.text(bar.get_x() + bar.get_width()/2, 
                    bar.get_height() + max(revenue_data.values)*0.03,  # More space above
        ...
        # Chart 2: Customer Age Segmentation (Middle Right) - FIXED PIE CHART
        ax2 = fig.add_subplot(gs[1, 2:])
        
        # PROPER pie chart with good spacing
        wedges, texts, autotexts = ax2.pie(
            ...
            startangle=90, 
            explode=[0.12]*len(age_segments),  # LARGER explode for separation
            shadow=True,                       # Add shadow for depth
            textprops={'fontsize': 11, 'fontweight': 'bold'}  # Better text
        )
        
        ax2.set_title('Customer Age Segmentation', fontsize=16, fontweight='bold', 
                     pad=25, color=self.colors['dark'])
        
        # Enhanced pie chart text formatting
        for autotext in autotexts:
            autotext.set_color('white')
            autotext.set_fontweight('bold')
            autotext.set_fontsize(12)
        
        # Chart 3: Top Customers (Bottom Left) - FIXED SPACING
        ...
        # Chart 4: Product Ratings (Bottom Right) - Better label visibility
        ...
        # BETTER x-axis labels - rotated for visibility
        ...
        # Save the final professional dashboard
        dashboard_path = f"{self.output_dir}/sales_performance_final_professional.png"
        plt.savefig(dashboard_path, dpi=300, bbox_inches='tight', 
                   facecolor='white', pad_inches=0.3)  # More padding
        plt.close()
        
        print(f"Final professional dashboard saved: {dashboard_path}")
        return dashboard_path
        
    def create_thumbnail_version(self):
        """Create matching professional thumbnail"""
        print("Creating professional thumbnail...")
        
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(12, 9))  # Larger thumbnail
        fig.patch.set_facecolor('white')
        
        # Smaller, professional title
        ...
        # Chart 2: Age segments - PROPER pie chart
        wedges, texts, autotexts = ax2.pie(
            age_segments.values, 
            labels=age_segments.index, 
            autopct='%1.0f%%',
            colors=colors[:len(age_segments)], 
            explode=[0.08]*len(age_segments),  # Good separation
...
Intelligence Dashboard"""
        ...
        # Save thumbnail
        thumb_path = f"{self.output_dir}/sales_performance_final_thumbnail.png"
        plt.savefig(thumb_path, dpi=200, bbox_inches='tight', 
                   facecolor='white', pad_inches=0.2)
        plt.close()
        
        print(f"Professional thumbnail saved: {thumb_path}")
        return thumb_path
        
    def generate_final_dashboard_suite(self):
        """Generate the final professional dashboard suite"""
        print("Creating FINAL Professional Dashboard Suite")
        print("=" * 60)
        
        try:
            results = {}
            
            results['main_dashboard'] = self.create_perfect_professional_dashboard()
            results['thumbnail'] = self.create_thumbnail_version()
            
            print("\n" + "=" * 60)
            print("SUCCESS: Final professional dashboard suite complete!")
            print(f"\nOutput directory: {os.path.abspath(self.output_dir)}")
            print("\nFinal files created:")
            print(f"1. Main Dashboard: sales_performance_final_professional.png")
            print(f"2. Thumbnail: sales_performance_final_thumbnail.png")
            
            print(f"\nFIXES APPLIED:")
            print(f"✅ Pie chart properly exploded with shadow")
            print(f"✅ Smaller, professional header text")
            print(f"✅ Fixed bottom chart label visibility")
            print(f"✅ Increased spacing between all elements")
            print(f"✅ Professional color scheme and typography")
            
            print(f"\nReady for portfolio upload!")
...
def main():
    dashboard = FinalProfessionalDashboard()
    results = dashboard.generate_final_dashboard_suite()
    
    if results:
        print(f"\nSUCCESS: Final professional dashboard ready!")
        print(f"Upload these files to replace the cramped version.")
